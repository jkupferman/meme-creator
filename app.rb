...
def convert text, source, destination, location, width
  ...
  cmd = "convert -fill white -stroke black -strokewidth 2 -background transparent -gravity center -size #{width}x120 -font #{fontpath} -weight Bold -pointsize 32 caption:#{text} #{source} +swap -gravity #{location} -composite #{destination}"
  `#{cmd}`
end
